...
<Card
  title={<>Reward Leaderboard <Info text="Shows reward tokens minted to departments over time." /></>}
  right={<button onClick={fetchRewardHistory} style={btn("ghost")}>Load History</button>}
>
  {Object.keys(rewardsByMonth).length === 0 ? (
  <div style={{ color: "var(--muted)" }}>No reward history.</div>
) : (
  Object.entries(rewardsByMonth).map(([month, rows]) => (
    <div key={month} style={{ marginBottom: 20 }}>
      <h3 style={{ marginBottom: 8 }}>{month}</h3>
      <TableRewards rows={rows} />
    </div>
  ))
)}
